...
class Main(object):
    ...
    def __init__(self, master):
        ...
        # images for ships
        glider = self.creat.create_image(48, 25, image=self.glider)
        lss = self.creat.create_image(48, 73, image=self.lss)
        sui = self.creat.create_image(48, 126, image=self.sui)
        f5 = self.creat.create_image(48, 175, image=self.f5)
        penta = self.creat.create_image(48, 256, image=self.penta)
        patern = self.creat.create_image(48, 349, image=self.patern)
        fireship = self.creat.create_image(49, 485, image=self.fireship)
...
# make an app on the top after run if argv_emulation is True in setup.py
if 'Darwin' in platform.system():
    script = 'tell application "System Events" to set frontmost of the \
    first process whose unix id is {} to true'.format(getpid())
    system("/usr/bin/osascript -e '{}'".format(script))
